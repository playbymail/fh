...
// potential_home_system is file-local state in planet.c.
var potential_home_system = FALSE

// resetPlanetState restores planet.c's file-local state to its initial value.
func resetPlanetState() {
	potential_home_system = FALSE
}

func createHomeSystemTemplates() int {
	earth_like := TRUE
	makeMiningEasier := TRUE
	for num_planets := 3; num_planets < 10; num_planets++ {
		filename := fmt.Sprintf("homesystem%d.dat", num_planets)
		fmt.Printf(" info: creating template '%s'...\n", filename)
		...
		filename = fmt.Sprintf("homesystem%d.txt", num_planets)
		fp, err := os.Create(filename)
		if err != nil {
			fmt.Fprintf(os.Stderr, "createHomeSystemTemplates: %s\n", err)
			fmt.Fprintf(os.Stderr, "error: unable to create template '%s'\n", filename)
			return 2
		}
...
func fix_gases(pl *planet_data_t) {
	var i, j, total, left, add_neutral int
...
func generate_planets(first_planet []*planet_data_t, num_planets, earth_like, makeMiningEasier int) {
	var diameter [10]int
	var g [10]int
	var gas [10][5]int
	var gas_percent [10][5]int
	var mining_difficulty [10]int
	var pressure_class [10]int
	var special [10]int
	var temperature_class [10]int
	...
	for planet_number := 1; planet_number <= num_planets; planet_number++ {
		...
		if make_earth != FALSE && (tc <= 11) {
			...
			nitro := i /* Save index for nitrogen. */
			i++
	...
	for i := 1; i <= num_planets; i++ {
		planetidx := i - 1
		current_planet := first_planet[planetidx]
	...
	if potential_home_system != FALSE {
		potential := 0
		for i := 1; i <= num_planets; i++ {
			current_planet := first_planet[i-1]
...
func LSN(current_planet, home_planet *planet_data_t) int {
	tc := current_planet.temperature_class - home_planet.temperature_class
	if tc < 0 {
		tc = -tc
	}
	ls_needed := 2 * tc /* Temperature class. */

	pc := current_planet.pressure_class - home_planet.pressure_class
	if pc < 0 {
		pc = -pc
	}
